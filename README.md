...
- 已创建 `users`、`user_sessions`、`files`、`artifacts`、`artifact_files`、`comments`、`tasks`、`task_participants`、`task_submissions`、`issues`、`competitions`、`competition_registrations`、`competition_reviews`、`competition_results` 十四张业务表；加上 Alembic 的 `alembic_version` 共十五张表（迁移头 `20260819_0007`）。
- `deploy/` 已提供单应用 Dockerfile 与 Docker Compose，Vue 生产构建可由 FastAPI 托管。
- `backend_c#/` 的 .NET 6 移植版已对齐 Python 版批次 1–15 全部后端接口与 CLI（任务、Issues、竞赛、管理动作、结构化日志、备份恢复，以及第二阶段任务/竞赛闭环、评审计分与个人工作台），47 项 xUnit 测试通过。
...
批次 7（业务决策与数据契约）已于 2026-08-24 完成：8 项业务决策全部确认，状态转换表、权限矩阵、表结构、接口契约、迁移方案（`20260819_0006`/`0007`）与测试清单固化在 [`docs/MkAIHub第二阶段批次7数据契约.md`](docs/MkAIHub第二阶段批次7数据契约.md)。
...
批次 14–15（C# 版重新对齐与回归修复）已完成：批次 14 将 C# 版对齐 Python 版全部后端功能——任务/竞赛闭环、评审计分、个人工作台与跨模块筛选，`Migrator.cs` 同步复刻迁移链至 `20260819_0007`，47 项 xUnit 测试通过；批次 15 修复存在草稿竞赛时员工看不到独立任务的问题，Python 与 C# 两后端同步修改并补齐回归测试。自此两后端接口行为保持一致。
