...
- 已创建 `users`、`user_sessions`、`files`、`artifacts`、`artifact_files`、`comments`、`tasks`、`task_participants`、`task_submissions`、`issues`、`competitions`、`competition_registrations`、`competition_reviews`、`competition_results` 十四张业务表；加上 Alembic 的 `alembic_version` 共十五张表（迁移头 `20260819_0008`）。`0008` 会为存量有效竞赛报名补齐竞赛任务参与关系。
- `deploy/` 已提供单应用 Dockerfile 与 Docker Compose，Vue 生产构建可由 FastAPI 托管。
- `backend_c#/` 的 .NET 6 移植版已对齐 Python 版批次 1–16 全部后端接口与 CLI（任务、Issues、竞赛、管理动作、结构化日志、备份恢复，以及第二阶段任务/竞赛闭环、评审计分与个人工作台），并同步至迁移头 `20260819_0008`。
- 两份项目方案已校准知识库边界：知识库与展品类型相互独立，首版占位是主动控制范围。

第二阶段 M2 已完成并通过验收，Python 版仍是 API 契约基准与开发主线。批次 7–11 已打通“竞赛包含多个任务，参赛人通过任务提交关联展品成果，再按任务评分汇总竞赛结果”的业务主链路；普通独立任务保留参与、提交和验收闭环。批次 11 的自动化测试、多账号浏览器流程、迁移升级及备份恢复演练均已通过，记录见 [`docs/MkAIHub第二阶段批次11验收记录.md`](docs/MkAIHub第二阶段批次11验收记录.md)。C# 版已于批次 14 重新对齐并自批次 15 起与 Python 版同步修改，后续增强计划见 [`docs/MkAIHub后续增强实施方案.md`](docs/MkAIHub后续增强实施方案.md)。

批次 7（业务决策与数据契约）已于 2026-08-24 完成：8 项业务决策全部确认，状态转换表、权限矩阵、表结构、接口契约、迁移方案（`20260819_0006`–`0008`）与测试清单固化在 [`docs/MkAIHub第二阶段批次7数据契约.md`](docs/MkAIHub第二阶段批次7数据契约.md)。

批次 8（任务与展品闭环）已完成：迁移 `20260819_0006` 扩展任务状态机（`IN_PROGRESS`/`REVIEWING`）并新增参与表与提交表；后端提供领取/退出、多轮提交、退回修改、验收/拒绝、完成约束与管理员重开接口，以及"我参与的""待我验收"筛选和展品反向来源查询；前端任务详情页改造为参与区、提交区、验收区和处理时间线，任务列表增加筛选，展品详情显示任务来源。

批次 9（竞赛、任务与展品闭环）已完成：迁移 `20260819_0007` 为任务表增加竞赛五列（归属、必做、排序、最高分、权重）、为竞赛表增加运营状态（`DRAFT`/`PUBLISHED`/`RESULT_PUBLISHED`/`ARCHIVED`），并新增报名、评审、结果三张表；后端提供报名/取消、竞赛任务配置（草稿自由、产生数据后锁定计分配置）、竞赛提交（报名 + 时间窗口校验、多轮改交、截止靠提交时校验锁定）、管理员评审（一条提交唯一评审、改分即更新）、结果发布（Decimal 加权计分、标准竞赛排名、快照冻结与整体重发布）、归档与删除预检接口；前端竞赛详情页提供报名、任务清单（必做/选做、分值、我的提交）与冻结排行榜，管理页提供任务配置、评审进度、结果发布与奖项维护，任务详情页内嵌竞赛评审，任务列表与展品来源显示竞赛归属与得分名次。

批次 10（待办、管理与跨模块体验）已完成：新增只读工作台聚合接口 `/api/v1/workbench`，探索页提供“我参与的任务”“我的竞赛任务”“待我验收”“待我评审”直达入口和可重算闭环统计；任务列表增加竞赛任务与管理员待评审筛选；展品列表增加任务成果、竞赛作品来源筛选与徽标。竞赛归档、评审进度、任务处理时间线和结构化管理日志复用批次 8/9 既有能力，本批次不新增数据库表或迁移。

批次 11（验收与交付）已完成：后端 44 项测试、前端 77 项测试、类型检查和生产构建通过；管理员与多名员工完成任务及竞赛核心浏览器流程；空库和首版库升级到 `20260819_0007`、逐级降级、外键约束和升级后备份恢复均已验证。

批次 12（M2 文档封板与后续规划）已完成：同步 README、项目规划和第二阶段方案的实际状态，明确下一批先统一批次 8–11 新增前端页面的视觉与交互样式，再安排检索与复用增强。知识库仍是占位页；当前安全继续保持轻量，不加入来源策略、限流、设备/IP 风控、病毒扫描或复杂管理员治理。

批次 13（前端样式统一与体验优化）已完成：统一批次 8–11 新增任务、竞赛、工作台与管理页面的详情布局、筛选区域和操作按钮样式，不改变业务规则、权限与 API 契约。

批次 14–15（C# 版重新对齐与回归修复）已完成：批次 14 将 C# 版对齐 Python 版全部后端功能——任务/竞赛闭环、评审计分、个人工作台与跨模块筛选，`Migrator.cs` 同步复刻迁移链至 `20260819_0007`；批次 15 修复存在草稿竞赛时员工看不到独立任务的问题，Python 与 C# 两后端同步修改并补齐回归测试。自此两后端接口行为保持一致。

批次 16（竞赛报名与任务展品闭环）已完成：报名竞赛时自动为用户创建或恢复该竞赛全部任务的有效参与关系；取消报名同步退出，竞赛任务不再允许绕过报名单独领取或退出。任务详情页可在当前页面直接新建并发布展品，成功后自动选中并提交成果，不再打开独立页面；Python 后端为主线，C# 后端与迁移同步维护。
...
MkAIHub/
  ...
  docs/                  # 产品规划、首版方案、第二阶段闭环方案与批次7数据契约
...
- 批次 8 闭环验证：`uv run pytest` 通过（34 项后端测试，含状态机、越权、可见性、约束与迁移用例）；`npm run type-check`、`npm run test`（42 项前端测试）和生产构建通过；本地演示库已从 `20260819_0005` 升级到 `20260819_0006`，存量任务保持开放状态、新表为空。C# 版按第二阶段决定停留在 `0005`，暂不能读写已升级的数据库。
- 批次 9 闭环验证：`uv run pytest` 通过（40 项后端测试，含竞赛全流程、计分与并列名次、配置锁定、约束与迁移升降级用例）；`npm run type-check`、`npm run test`（50 项前端测试）和生产构建通过；本地演示库已从 `20260819_0006` 升级到 `20260819_0007`，存量竞赛自动视为已发布（`PUBLISHED`），外键完整性检查为空。
- 批次 10 体验验证：`uv run pytest` 通过（41 项后端测试，新增个人待办、闭环统计、任务与展品跨模块筛选用例）；`npm run type-check`、`npm run test`（51 项前端测试）和生产构建通过。本地前后端启动及登录页访问通过；登录后的多账号核心流程按批次 11 统一验收。
