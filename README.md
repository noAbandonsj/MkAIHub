...
- 已创建 `users`、`user_sessions`、`files`、`artifacts`、`artifact_files`、`comments`、`tasks`、`task_participants`、`task_submissions`、`issues`、`competitions` 十一张表（迁移头 `20260819_0006`）。
...
批次 8（任务与展品闭环）已完成：迁移 `20260819_0006` 扩展任务状态机（`IN_PROGRESS`/`REVIEWING`）并新增参与表与提交表；后端提供领取/退出、多轮提交、退回修改、验收/拒绝、完成约束与管理员重开接口，以及"我参与的""待我验收"筛选和展品反向来源查询；前端任务详情页改造为参与区、提交区、验收区和处理时间线，任务列表增加筛选，展品详情显示任务来源。竞赛相关能力按契约留给批次 9。
