...
- 已创建 `users`、`user_sessions`、`files`、`artifacts`、`artifact_files`、`comments`、`tasks`、`task_participants`、`task_submissions`、`issues`、`competitions`、`competition_registrations`、`competition_reviews`、`competition_results` 十四张表（迁移头 `20260819_0007`）。
...
当前处于第二阶段，开发主线优先完善 Python 版。批次 7–10 已完成，业务主链路已经调整为“竞赛包含多个任务，参赛人通过任务提交关联展品成果，再按任务评分汇总竞赛结果”；普通独立任务保留参与、提交和验收闭环。下一步进入批次 11 验收与交付，C# 版继续暂缓同步。问题定义、实施边界、研发批次和验收标准见 [`docs/MkAIHub第二阶段闭环实施方案.md`](docs/MkAIHub第二阶段闭环实施方案.md)。
...
数据库与上传文件的备份、恢复已实现并完成一次实际恢复演练（见 `deploy/BACKUP.md`）。首版简化交付不包含容器重启和浏览器自动化，但第二阶段核心闭环的多账号浏览器验收已经明确列入批次 11；知识库仍是占位页。首版安全保持轻量，只实现密码哈希、服务端会话、HttpOnly Cookie、简单 CSRF、角色校验和附件的大小/扩展名基础限制，不加入来源策略、限流、设备/IP 风控、病毒扫描或复杂管理员治理。
