...
- 2026-08-26（批次 11 升级后演练）：使用独立演练目录不触碰正式演示库。空库直接升级到 `20260819_0007`（15 张表、外键检查为空）；首版演示库副本（`0005`，3 用户 / 3 展品 / 2 任务）升级到 `0007` 后数据无损，存量任务保持独立任务。升级后执行 `backup`（数据库 + 上传目录），随后新增管理员账号、上传目录新增文件并删除既有文件；`restore --yes` 后账号数与迁移版本回到备份点（3 用户、`0007`），被删文件还原，备份后新增文件按上述覆盖式语义保留；再按文档说明清空上传目录重新恢复后仅剩备份时内容。
