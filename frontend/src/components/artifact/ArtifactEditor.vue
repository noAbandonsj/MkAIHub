...
const emit = defineEmits<{
  saved: [artifact: ArtifactRead, published: boolean]
}>()
...
async function loadArtifact(): Promise<void> {
  ...
  if (!Number.isInteger(props.artifactId) || Number(props.artifactId) <= 0) {
    errorMessage.value = '展品编号无效'
    return
  }
  loading.value = true
  try {
    const artifact = await artifactsApi.get(Number(props.artifactId))
...
async function save(publishAfterSave: boolean): Promise<void> {
  ...
  try {
    ...
    let artifact = isEditing.value
      ? await artifactsApi.update(Number(props.artifactId), input)
    ...
    emit('saved', artifact, publishAfterSave)
  } catch (error) {
    ...
    <div v-else-if="!isEditing || loadedArtifact" class="artifact-form-layout">
      <section class="panel-card artifact-editor-panel">
        ...
        <div class="form-actions">
          <ElButton v-if="allowDraft" :loading="submitting" @click="save(false)">
