...
> 文档状态：首版实施基线（批次 0–5 已按简化范围收尾）
> 版本：v0.8
> 更新日期：2026-08-21
> 前置文档：[MkAIHub 项目规划](./MkAIHub项目规划.md)
> 后续文档：[MkAIHub 第二阶段闭环实施方案](./MkAIHub第二阶段闭环实施方案.md)
...
分批实施时，批次 2 的 `comments` 先只包含必填的 `artifact_id`；批次 3 引入 Issues 时再通过 Alembic 增加 `issue_id`、调整可空性并补充“二者恰好一个”的检查约束，避免提前创建空的 Issue 业务表。
...
1. 校验登录和 CSRF。
2. 校验请求大小、文件大小和附件数量。
3. 校验扩展名并记录客户端声明的 MIME；首版不做文件头识别、病毒扫描或压缩包解析。
4. 生成随机文件名并写入临时文件。
5. 计算 SHA-256。
6. 移动到受控目录并保存元数据。
7. 保存展品时建立附件引用。
...
任务、展品和竞赛闭环已经进入第二阶段规划，本文件继续保留首版边界和历史实施基线，不在此处追加第二阶段的数据表、接口和状态机。后续实施以 [MkAIHub 第二阶段闭环实施方案](./MkAIHub第二阶段闭环实施方案.md) 为准。
