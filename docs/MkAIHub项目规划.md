...
- 当前进入 M2，第一优先级是打通“竞赛—竞赛任务—任务提交—展品成果—评审结果”主链路，同时保留普通独立任务的参与、提交和验收闭环。
- M2 不与全文检索、SSO、基础设施扩容或 AI 能力同时展开，先完成核心业务关系和状态机。
...
5. 闭环稳定后，再根据内部使用反馈评估全文检索、分享版本、内容统计、文件预览和公司 SSO。
...
首版工程已经完成。第二阶段开始数据库迁移和接口实现前，需要确认以下业务规则：

1. 任务参与是员工直接领取，还是申请后由任务发布人确认。
2. 任务成果和竞赛作品在提交时必须已经发布，还是允许先以受限可见状态评审、通过后再发布。
3. 普通独立任务是否允许多人分别提交，以及发布人是否可以验收多个成果；竞赛任务已确认允许多轮提交并只取最终有效版本计分。
4. 竞赛首期由系统管理员直接评审，还是需要单独配置评委、评分项和权重。

这些问题不阻塞当前文档规划，但会影响状态机、权限、数据表和迁移设计，必须在批次 7 编码前形成明确结论。
